...
class TranscriberService:
    """OPTIMIZED for Render - Fast with memory efficiency"""
    
    def __init__(self):
        # ✅ Reduce threads for Render's 512MB memory limit
        self.executor = ThreadPoolExecutor(max_workers=1)
        
        # ✅ Use environment variable for model size (default: tiny for Render)
        model_size = os.environ.get("WHISPER_MODEL_SIZE", "tiny")
        logger.info(f"Loading Whisper model: {model_size} (optimized for Render)")
        
        self.model = WhisperModel(
            model_size,  # ✅ Use env var (tiny=75MB, base=150MB, small=500MB)
            device="cpu",
            compute_type="int8",
            cpu_threads=2,  # ✅ Reduced from 4 to 2 for memory
            num_workers=1   # ✅ Keep at 1 for Render
        )
        logger.info(f"✅ Model ready - {model_size}")
    ...
    async def transcribe(self, audio_data: bytes, language: str = "auto") -> Dict[str, Any]:
        ...
        try:
            ...
            # ✅ Optimized parameters for speed on Render
            segments, info = await loop.run_in_executor(
                self.executor,
                lambda: self.model.transcribe(
                    temp_path,
